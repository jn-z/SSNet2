...
class Creat_DlaNet(nn.Module):
    def __init__(self, base_name, heads,
                 pretrained=True, plot=False, down_ratio=2, head_conv=256):
        ...
        for head in self.heads:
            ...
            if head_conv > 0 :#and {'cl_id','con_id'} not in head:
                fc = nn.Sequential(
    ...
    def forward(self, x):
        x = self.base(x)
        #pdb.set_trace()
        x = self.dla_up(x[self.first_level:]) #torch.Size([2, 32, 256, 256])
        # x = self.fc(x)
...
def DlaNet(num_layers=34, heads = {'hm': 4, 'h_ud': 1, 'reg_y': 1}, head_conv=256, plot=False):
